...
class ConfigureLinkedInConnection {
  ...
  logoutFromLinkedIn = () => {
      IN.User.logout(function () {
          console.log("User logged out from LinkedIn");
      });
  };

  getLinkedInUserInfo = () => {
      IN.API.Raw("/me").result(function (response) {
          console.log("LinkedIn User Info", response);
      });
  };
  ...
  processPagesList = (response) => {
      if (response.elements && response.elements.length) {
          let pagesList = {};
          let pagesListOptionsHtml = '<div class="btn-group-vertical" role="group">';
          let linkedInUserId = this.userId;

          response.elements.forEach((pageData) => {
          ...
          let connection_name = this.connection_configuration_toggle_element.attr("data-connection_setting_name");
          ...
          Swal.fire({
              ...
              html:
                  ...
                  '<input type="hidden" name="connection_type" value="linkedin_page">\n' +
          ...
          disableConfigurationAndDisableActiveConnection(
              ...
              "You don't have any LinkedIn pages available"
          );
      }
      this.logoutFromLinkedIn();
  };

  getLinkedInPages = () => {
      IN.API.Raw("/organizationalEntityAcls?q=roleAssignee&role=ADMIN").result((response) => {
          this.processPagesList(response);
      });
  };
  ...
  processLoginResponse = (response) => {
      if (response) {
          ...
          disableConfigurationAndDisableActiveConnection(
              this.connection_configuration_toggle_element,
              this.connection_selector,
              "You have cancelled LinkedIn login"
          );
      }
  };
  ...
  showLinkedInLogin = () => {
      IN.User.authorize(() => {
          ...
          scope: ["r_liteprofile", "r_emailaddress", "r_organization_admin", "w_organization_social"],
          state: true
      });
  };

  checkLoginStatus = () => {
      IN.User.isAuthorized() ? this.getLinkedInPages() : this.showLinkedInLogin();
  };
}
